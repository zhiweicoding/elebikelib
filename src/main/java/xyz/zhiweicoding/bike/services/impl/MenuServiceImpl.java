...
import xyz.zhiweicoding.bike.dao.mysql.*;
...
import xyz.zhiweicoding.bike.services.MenuService;
import xyz.zhiweicoding.bike.support.RedisSupport;
import xyz.zhiweicoding.bike.utils.GeneratorUtils;
...
public class MenuServiceImpl extends ServiceImpl<MenuDao, MenuBean> implements MenuService {
    ...
    public void delAllById(String menuId) {
        ...
        for (String m : ms) {
            imgDao.delete(Wrappers.<ImgBean>lambdaQuery().eq(ImgBean::getMenuId, m));
            contentDao.delete(Wrappers.<ContentBean>lambdaQuery().eq(ContentBean::getMenuId, m));
        }
    ...
    private void addSons(List<String> stores, String oldId) {
        Long hasSons = menuDao.selectCount(Wrappers.<MenuBean>lambdaQuery().eq(MenuBean::getMenuFather, oldId).eq(MenuBean::getIsDelete, 0));
        if (hasSons > 0) {
            List<MenuBean> fs = menuDao.selectList(Wrappers.<MenuBean>lambdaQuery().eq(MenuBean::getMenuFather, oldId).eq(MenuBean::getIsDelete, 0));
            for (MenuBean f : fs) {
                stores.add(f.getMenuId());
                String menuId = f.getMenuId();
                addSons(stores, menuId);
            }
        }
    }
