...
public class GoodController {
    ...
    @PostMapping("/detail")
    public @ResponseBody BaseResponse<GoodBean> detail(@RequestBody IndexVo param) {
        log.debug("获取商品的详细信息,入参 : {}", JSON.toJSONString(param));
        try {
            GoodBean detail = goodService
                    .getOne(Wrappers.<GoodBean>lambdaQuery().eq(GoodBean::getGoodId, param.getGoodId()));
